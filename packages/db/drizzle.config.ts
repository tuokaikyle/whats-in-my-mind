...
if (!url) {
  throw new Error(
    dbTarget === 'production'
      ? 'Production migration URL is not set. Set DATABASE_URL_PRODUCTION_DIRECT, DATABASE_URL_PRODUCTION, DATABASE_URL, or DATABASE_URL_MIGRATE.'
      : 'Development migration URL is not set. Set DATABASE_URL_DEV_DIRECT, DATABASE_URL_DIRECT, DATABASE_URL_DEV, DATABASE_URL, or DATABASE_URL_MIGRATE.',
  );
}
